...
public class SugestaoDeJogoService {
    ...
    public Page<SugestaoDeJogo> buscarSugestoesPaginadas(String termo, Pageable pageable) {
        if (termo != null && !termo.trim().isEmpty()) {
            return sugestaoDeJogoRepository.buscarPorTermo(termo, pageable);
        } else {
    ...
    public void marcarComoFeito(String id) {
        Optional<SugestaoDeJogo> optional = sugestaoDeJogoRepository.findById(id);
        if (optional.isEmpty()) {
            throw new RuntimeException("Sugestão não encontrada");
        }

        SugestaoDeJogo sugestao = optional.get();
        sugestao.setFeito(true); // marca como feito
        sugestaoDeJogoRepository.save(sugestao);
    }
